...
/**
 * @param {{ es: import('../simulation/settings').EditorSystem }} deps
 */
export const createGeometryHelpers = ({ es }) => ({
  ...
  pointFromEvent: (event, svgEl) => {
    ...
    return {
      x: (sx - view.position.x) / view.zoom,
      y: (sy - view.position.y) / view.zoom,
    };
  },
  ...
  worldToScreen: (wx, wy) => {
    const view = es.viewport();
    return {
      x: view.position.x + wx * view.zoom,
      y: view.position.y + wy * view.zoom,
    };
  },
